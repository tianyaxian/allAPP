...
var iVolume = 50;
/*音量+10*/
function setVolume1(ckobj){
	if(iVolume<=100) iVolume+=10;
	else iVolume=100;
	ckobj.changeVolume(iVolume);
	console.log("volume="+iVolume);
}
/*音量-10*/
function setVolume2(ckobj){
...
/*循环播放视频处理程序*/
function playerstop(){
	if(urlFlag==flvdata.length-1) urlFlag=0;
	else urlFlag++;
	playVideo(flvdata[urlFlag]);
}
function playVideo(url){
	...
	CKobject.embedSWF('/apps/dvideo_local/ckplayer/ckplayer.swf','b1','ckplayer_b1','742','420',flashvars,params);
}
/*
插件的定义结束
以下是对播放器功能进行配置
