...
(function ($) {
    ...
    displayBooks = function (dom, data) {
        ...
        $(data.list).each(function (idx) {
            var li = $("<li/>").attr("bookNum", this.bookNum), bookshelf = $(idx < 7 ? "ul:eq(0)" : "ul:eq(1)", dom),
                ...
                qrimg = $("<img/>").addClass("erwei").attr("src", "/server/qr?gedecache=3&qr=" + qrpath).load(function () {
                    $(this).show();
                }).hide();
    ...
    !(loadBooks = function (id, page, fn) {
        ...
        window.loadBooksTimeid = window.setTimeout(function(){
            ...
            $.ajax({
                type: "get",
                dataType: "json",
                url: "/server/apps/touchbook?gedecache=" + window.GEDE_VERSION + "&nocache=1&cmd=books&pageSize=14&id=" + (_CID = id) + "&page=" + (page == null ? 1 : page),
    ...
    !function loadCatas() {
        $.ajax({
            type: "get",
            dataType: "json",
            url: "/server/apps/touchbook?cmd=bookCata&nocache=1&gedecache=" + window.GEDE_VERSION,
            success: function (data) {
